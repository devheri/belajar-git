...
public class DataHandler2 {
    ...
    public DataHandler2(String lookupKey) {
        super();
        this.lookupKey = lookupKey;
    }
    ...
    private List<ParseElement> processParse(String lookupKey, StringBuffer sb, List<?> p) {
        ...
        if (bitMap != null) {
            ...
            editor.putString("DataBitmap",bitMap.getValue().toString());
...
//                        + mirror.getValue() : mirror.getId() + "域" + ":" + mirror.getValue());
                ...
                parseList.add(mirror);
                stringBuilder.append("Bit "+mirror.getId() +" : "+mirror.getValue()+"\n");



            }
                editor.putString("DataElement",stringBuilder.toString());
                editor.commit();
        }
